...
if(loginUser != null) {
    ...
    const userAddress = loginUser.userAddress.split("^^^");

    if(userAddress.length != 0) {

        postcode.value = userAddress[0];
        address.value = userAddress[1];
        detailAddress.value = userAddress[2];
    }
...
document.getElementById("orderBtn").addEventListener("click", e => {
    ...
    const userAddressList = document.querySelectorAll("[name=userAddress]");

    const userAddress = [];
        userAddressList.forEach(e => {
            userAddress.push(e.value);
        });

    order.orderAddressList = userAddress;
