...
export class InputSpinner {
  constructor(element, props) {
    ...
    if (this.element['bootstrap-input-spinner']) {
      ...
      this.$original.hide();
      this.$original[0].inputSpinnerEditor = new this.props.editor(
        this.props,
        this.element
      );
      ...
      this.$input
        .on('paste input change focusout', function (event) {
          ...
          if (self.props.keyboardStepping && focusOut) {
            // stop stepping
            resetTimer();
          }
        })
    ...
    function setValue(newValue, updateInput) {
      ...
      if (isNaN(newValue) || newValue === '') {
        ...
        if (updateInput) {
          self.$input[0].value =
            self.$original[0].inputSpinnerEditor.render(newValue);
        }
        self.value = newValue;
      }
    }
    ...
    function updateAttributes() {
      ...
      self.$input.attr(
        'inputmode',
        self.$original.attr('inputmode') || 'decimal'
      );
      ...
      self.$inputGroup.prop(
        'class',
        'input-group ' + groupClass + ' ' + self.props.groupClass
      );
      self.$input.prop('class', 'form-control ' + inputClass);

      // update the main attributes
      self.min =
        isNaN(self.$original.prop('min')) || self.$original.prop('min') === ''
          ? -Infinity
          : parseFloat(self.$original.prop('min'));
      self.max =
        isNaN(self.$original.prop('max')) || self.$original.prop('max') === ''
          ? Infinity
          : parseFloat(self.$original.prop('max'));
